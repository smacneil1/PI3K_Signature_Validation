...
outputFileName_old= "~/Documents/PhDProjects/PI3K_Signature_Validation/results/ssGSEA_PI3K_old.txt"
...
write.table(GeneSets_pi3k_all, outputFileName_all, col.names=T, quote=FALSE, sep="\t")
write.table(GeneSets_pi3k_rm5610, outputFileName_rm, col.names=T, quote=FALSE, sep="\t")

#now do it on andreas signature

gsva_pi3k_old_sig <- gsva(as.matrix(SCAN_gfp_pi3k), c2set, min.sz=10, max.sz=500, verbose=TRUE, rnaseq=F, method="ssgsea")

colnames_pi3k=as.data.frame(colnames(SCAN_gfp_pi3k))
colnames_pi3k

classes=(c( "pi3k","pi3k","pi3k","pi3k","pi3k","pi3k", "pi3k", "pi3k", "control","control","control","control","control","control","control","control","control","control","control"))
classes=as.matrix(classes)
classes
classes=cbind(colnames_pi3k,classes)
rownames(classes)=classes[,1]
...
design_old <- model.matrix(~ factor(classes[,2]))
design_old
colnames(design_old) <- c("ALL", "PI3KvsGFP")
old_fit <- lmFit(gsva_pi3k_old_sig, design_old)
old_fit  <- eBayes(old_fit)
GeneSets_pi3k_old <- topTable(old_fit, coef="PI3KvsGFP", number=Inf,adjust.method = "BH", sort.by = "p" )

View(GeneSets_pi3k_old)
write.table(GeneSets_pi3k_old , outputFileName_old, col.names=T, quote=FALSE, sep="\t")
